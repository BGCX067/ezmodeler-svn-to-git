...
import org.i4change.app.hibernate.beans.user.Users_Usergroups;
import org.i4change.app.hibernate.beans.user.Usergroups;
 ...
 *
 */
public class Groupmanagement extends HibernateDaoSupport {
	...
	public String updateUserGroup(Long user_level, Long users_usergroups_id,
		...
		if (checkUserLevel(user_level)) {
			try {
				...
				int updatedEntities = getSession().createQuery(hqlUpdate).setLong(
	...
	public Usergroups[] getUsergroupsUsers(Long usergroup_id) {
		...
		try {
			...
			for (int vars = 0; vars < usergroups.length; vars++) {
				...
				//usergroups[vars].setUsers(ResHandler.getUserDaoImpl().getUserForGroup(usergroups[vars].getUSER_ID()));
			}
		} catch (HibernateException ex) {
			log.error(ex);
		} catch (Exception ex2) {
	...
	public Usergroups[] getAllGroup(Long user_level) {
		...
		if (checkConfLevel(user_level)) {
			try {
				...
				for (int vars = 0; vars < usergroups.length; vars++) {
					//groups[vars].setUsers(ResHandler.getUserDaoImpl().getUser(groups[vars].getUSER_ID()));
				}
			} catch (HibernateException ex) {
				log.error(ex);
			} catch (Exception ex2) {
	...
	public Users_Usergroups getSingleGroup(Long user_level, Long usergroup_id) {
		...
		if (checkConfLevel(user_level)) {
			try {
				...
				//groups.setUsers(ResHandler.getUserDaoImpl().getUser(groups()));
	...
	public Usergroups getSingleGroupFree(Long user_level, Long usergroup_id) {
		...
		if (checkConfLevel(user_level)) {
			try {
				...
				//Todo: Set user
				//groups.setUsers(ResHandler.getUserDaoImpl().getUser(groups.getUSER_ID()));
