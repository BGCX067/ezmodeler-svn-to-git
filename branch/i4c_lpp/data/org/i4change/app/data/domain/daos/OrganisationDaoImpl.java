...
public class OrganisationDaoImpl extends HibernateDaoSupport {
	...
	public Long addOrganisation(String orgname, long user_id) {
		try {
			...
			long id = (Long) getSession().save(org);
			return id;
...
//					
//					List<Users> modUsers = OrganisationUserDaoImpl.getInstance().getModeratorsByOrganisationId(org.getOrganisation_id());
//					
...
//						}
//						
//						String reminderUsers = mod.getLogin() + " (" + mod.getUser_id() + ") (" + mail.getMail().getEmail() + ") ("+mod.getReceivePendingReminder()+")";
//						
//
...
//						sendedReminderOrg.put("orgCreationDate", CalendarPatterns.getDateWithTimeByMiliSeconds(org.getStarttime()));
...
//						
//						if (mod.getReceivePendingReminder() == null || mod.getReceivePendingReminder()) {
//							Mailmanagement.getInstance().addMailToSpoolAboutPendingOrganization(mod.getUser_id(), receipent, 
...
//	}
	 ...
	 * @param user_id
	 */
	public Long addOrganisationWithMap(String orgname, long user_id, Vector orgPatternsMap) {
		try {
	 ...
	 * @return
	 */
	public SearchResult getOrganisations(int start ,int max, String orderby, boolean asc) {
		try {
			SearchResult sresult = new SearchResult();
			sresult.setObjectName(Organisation.class.getName());
			sresult.setRecords(this.selectMaxFromOrganisations());
			
			List<Organisation> orgList = this.getOrganisationsList(start, max, orderby,asc);
			...
			for (int i=0;i<orgList.size();i++) {
				...
				organisationListDTO.setOrganisation_id(org.getOrganisation_id());
	 ...
	 * @return
	 */
	public SearchResult getPendingOrganisations(int start ,int max, String orderby, boolean asc) {
		try {
			SearchResult sresult = new SearchResult();
			sresult.setObjectName(Organisation.class.getName());
//			sresult.setRecords(this.selectMaxFromPendingOrganisations());
//			sresult.setResult(this.getPendingOrganisationsList(start, max, orderby,asc));
			return sresult;
		} catch (HibernateException ex) {
			log.error("[getOrganisations]" ,ex);
		} catch (Exception ex2) {
	 ...
	 * @return
	 */
	public List<Organisation> getOrganisationsList(int start ,int max, String orderby, boolean asc) {
		try {
...
//	public List<Organisation> getPendingOrganisationsList(int start ,int max, String orderby, boolean asc) {
//		try {
//			Object idf = HibernateUtil.createSession();
//			Session session = HibernateUtil.getSession();
//			Transaction tx = session.beginTransaction();
...
//				List<Organisation> ll = crit.list();
	...
	private Long selectMaxFromOrganisations(){
		try {
			//get all users
			Query query = getSession().createQuery("select max(c.organisation_id) from Organisation c where c.deleted = 'false'"); 
			List ll = query.list();
...
//	}	
	...
	private Long updateOrganisationUsersByHashMap(Organisation org , LinkedHashMap users, long insertedby){
		try {
			...
			List usersStored = this.organisationUserDaoImpl.getUsersByOrganisationId(org.getOrganisation_id());
			...
			for (Iterator<Long> it = usersToAdd.iterator();it.hasNext();){
				...
				this.addUserToOrganisation(user_id, org.getOrganisation_id(), insertedby, "", false);
			}
...
//				}
				...
				long id = (Long) getSession().save(orgUser);
				return id;
	...
	public Long addUserToOrganisationByModerator(Long user_id, Long organisation_id) {
		try {
			if (this.getOrganisation_UserByUserAndOrganisation(user_id, organisation_id)==null) {
...
//						Long default_lang_id = Long.valueOf(Configurationmanagement.getInstance().getConfKey(3,"default_lang_id").getConf_value()).longValue();
//		        		
//						//send Mail to Org.-Moderators and Sys-Administrators about the failed auto-assign action
//						List<Users> orgModUsers = OrganisationUserDaoImpl.getInstance().getModeratorsByOrganisationId(organisation_id);
//						
...
//							Mailmanagement.getInstance().addMailToSpoolAboutFailedAutoAssign(user_id, us.getUser_id(), 
//									orga.getName(), us.getFirstname()+" "+us.getLastname()+" ["+us.getLogin()+"]", email, default_lang_id);
//						}
//						
//						List<Users> sysAdminUsers = UserDaoImpl.getInstance().getAdmins();
//						
//						for (Users us : sysAdminUsers) {
//							Mailmanagement.getInstance().addMailToSpoolAboutFailedAutoAssignToSysAdmin(user_id, us.getUser_id(), 
//									orga.getName(), us.getFirstname()+" "+us.getLastname()+" ["+us.getLogin()+"]", email, default_lang_id);
//						}
//						
//						return new Long(-47);
...
//				}
	 ...
	 * @return
	 */
	public List<Organisation> getOrganisationsByUserId(long user_level, long user_id, int start, int max, String orderby, boolean asc){
		try {
			if (AuthLevelmanagement.checkAdminLevel(user_level)){
					Criteria crit = getSession().createCriteria(Organisation_Users.class);
					
					//TODO: I do not really remember that code, but it must be that way
	...
	public List getRestOrganisationsByUserId(long user_level, long user_id, int start, int max, String orderby, boolean asc){
		try {
			if (AuthLevelmanagement.checkAdminLevel(user_level)){
				...
				List allOrgs = this.getOrganisationsList(0, 1000000, orderby, asc);
	...
	public Long updateUserOrganisationsByUser(Users us, Vector organisations){
		try {
			...
			for (Iterator it = us.getOrganisation_users().iterator();it.hasNext();){
				...
				log.debug("updateUserOrganisationsByUser check1 : "+orgIdStored);
				boolean shouldBeStored = this.checkOrgInList(orgIdStored,organisations);
				if (!shouldBeStored) orgIdsToDelete.add(orgIdStored);
			}
		
			log.debug("updateUserOrganisationsByUser size ADD: "+orgIdsToAdd.size());
			for (Iterator it = orgIdsToAdd.iterator();it.hasNext();){
				Long orgToAdd = (Long) it.next();
				this.addUserToOrganisation(us.getUser_id(), orgToAdd, us.getUser_id(), "", false);
			}

			log.debug("updateUserOrganisationsByUser size DELETE: "+orgIdsToDelete.size());
		...
		try {
			if (org!=null){
				for (Iterator it = org.iterator();it.hasNext();){
					Object key = it.next();
					Long newOrgId = Long.valueOf(key.toString()).longValue();
					this.addUserToOrganisation(us, newOrgId, new Long(1), "", false);
				}
			}
		} catch (Exception ex) {
