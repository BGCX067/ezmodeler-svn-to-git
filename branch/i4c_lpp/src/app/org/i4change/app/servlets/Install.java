...
public class Install extends VelocityViewServlet {
	...
	public Template handleRequest(HttpServletRequest httpServletRequest,
		...
		try {
			...
			ImportInitvalues importInitvalues = (ImportInitvalues) context.getBean("i4change.ImportInitvalues");
			...
			if (command == null){
				...
				if (!installerFile.exists()){
					...
					if (!b) {
						...
						InstallationDocumentHandler.createDocument(working_dir+InstallationDocumentHandler.installFileName,0);
						//File has been created so follow first step of Installation					
						return getVelocityEngine().getTemplate("install_welcome_"+lang+".vm");
					}
					
				} else {
					int i = InstallationDocumentHandler.getCurrentStepNumber(working_dir);
					if (i == 0){
						String filePath = getServletContext().getRealPath("/")+IImportInitvalues.languageFolderName;
						LinkedHashMap<Integer,String> allLanguages = importInitvalues.getLanguageFiles(filePath);
				...
				if (i == 0){
					...
					//update to next step
					//InstallationDocumentHandler.createDocument(working_dir+InstallationDocumentHandler.installFileName,1);
				...
				if (i == 0){
					...
					String url_feed =  "http://groups.google.com/group/i4change/feed/atom_v1_0_msgs.xml";
					String url_feed2 =  "http://groups.google.com/group/i4change/feed/atom_v1_0_msgs.xml";
					...
					importInitvalues.loadConfiguration(crypt_ClassName, configdefault, configsmtp, configsmtpport, 
					...
					importInitvalues.loadInitUserAndOrganisation(username, userpass, useremail, orgname);
