...
public class LoadAtomRssFeed {
	...
	private static final Log log = LogFactory.getLog(LoadAtomRssFeed.class);
...
//	}
	...
	public LinkedHashMap<String,LinkedHashMap<String,LinkedHashMap<String,LinkedHashMap<String,Object>>>> getRssFeeds(Long user_level){
		try {
			if (AuthLevelmanagement.checkUserLevel(user_level)){
				...
				String url1 = this.configurationmanagement.getConfKey(3,"rss_feed1").getConf_value();
				returnMap.put("feed1", this.parseRssFeed(url1));
				
				String url2 = this.configurationmanagement.getConfKey(3,"rss_feed2").getConf_value();
	...
	public LinkedHashMap<String,LinkedHashMap<String,LinkedHashMap<String,Object>>> getRssFeedSingleStored(Long user_level){
		try {
			if (AuthLevelmanagement.checkUserLevel(user_level)){
				
				String url1 = this.configurationmanagement.getConfKey(3,"rss_feed1").getConf_value();
				return this.parseRssFeed(url1);
				
			} else {
				log.error("[getRssFeedSingleStored] authorization required");
			}
			
		} catch (Exception ex) {
			log.error("[getRssFeedSingleStored]",ex);
		}
		return null;
	}
