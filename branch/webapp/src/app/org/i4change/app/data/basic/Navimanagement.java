...
import org.i4change.app.hibernate.Naviglobal;
import org.i4change.app.hibernate.Navimain;
import org.i4change.app.hibernate.Navisub;
...
public class Navimanagement {
	...
	public List getMainMenu(long user_level, long USER_ID, long language_id) {
		...
		for (Iterator it2 = ll.iterator(); it2.hasNext();) {
			...
			navigl.setLabel(Fieldmanagment.getInstance().getFieldByLabelNumberAndLanguage(navigl.getLabel_number(),language_id));
			Set s = navigl.getMainnavi();
			for (Iterator it3 = s.iterator(); it3.hasNext();) {
				Navimain navim = (Navimain) it3.next();
				navim.setLabel(Fieldmanagment.getInstance().getFieldByLabelNumberAndLanguage(navim.getLabel_number(),language_id));
				for (Iterator it4 = navim.getSubnavi().iterator(); it4.hasNext();) {
					Navisub navis = (Navisub) it4.next();
					navis.setLabel(Fieldmanagment.getInstance().getFieldByLabelNumberAndLanguage(navis.getLabel_number(),language_id));
				}
