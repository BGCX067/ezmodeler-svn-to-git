...
public class ImportInitvalues {
	...
	public void loadMainMenu() {
		...
		UserDaoImpl.getInstance().addUserLevel("User", 1);
		UserDaoImpl.getInstance().addUserLevel("Moderator", 2);
		UserDaoImpl.getInstance().addUserLevel("Admin", 3);
		
		Long global_id = Navimanagement.getInstance().addGlobalStructure(
		...
		Long global_id_mod = Navimanagement.getInstance().addGlobalStructure(
								"mod", 2, 689, false, true, 2, "admin");
		Navimanagement.getInstance().addMainStructure("modRoles", 1, 690,
				true, false, 2, "modRoles", global_id_mod);
		Navimanagement.getInstance().addMainStructure("modRoles", 2, 691,
				true, false, 2, "modStatistics", global_id_mod);
		
		Long global_id_admin = Navimanagement.getInstance().addGlobalStructure(
	...
	public void loadConfiguration(String crypt_ClassName,
		...
		Configurationmanagement.getInstance().addConfByKey(3, "expireDateMonthsMax",
				"6", null, "Number of Months you can use your workdays for Pending Organizations");
		
		Configurationmanagement.getInstance().addConfByKey(3, "maxWorkDays",
				"30", null, "Number of workdays for Pending Organizations");
		
		Configurationmanagement.getInstance().addConfByKey(3, "maxUsers",
				"10", null, "Number of maxUsers for Pending Organizations");
		
		Configurationmanagement.getInstance().addConfByKey(3, "mailSpoolCheckTime",
				"10000", null, "Amount of Time in Milliseconds the Mail Spool will try to load the next Items from the Queue");
		
		Configurationmanagement.getInstance().addConfByKey(3, "mailSpoolMaxNumberOfItemsPerSpool",
				"5", null, "Number of mails which the Spool will check for Sending in one bunch");
		
		Configurationmanagement.getInstance().addConfByKey(3, "system_link_buy_license",
				"http://localhost/i4change/BuyLicense", null, "Link inside the Mails for Pending Organizations");
		
		Configurationmanagement.getInstance().addConfByKey(3, "system_link_unregister_from_list",
				"http://localhost:5080/i4change/Unregister", null, "Servlet to remove user from List of Pending Organizations");
		
		Configurationmanagement.getInstance().addConfByKey(3, "sessionClearCheckTime",
				"30000", null, "Time in MilliSeconds the Session cache will be cleared");
		
		Configurationmanagement.getInstance().addConfByKey(3, "reminderMailsPendingOrgs",
				"43200000", null, "Time in MilliSeconds the System will check for Pending Orgs to send a Reminder (86400000 == 1 Day)");
		
		
		Configurationmanagement.getInstance().addConfByKey(3, "defaultOrganizationPricing",
				"39.95", null, "default Price in USD for a Organization per Year + 1 account");
		
		Configurationmanagement.getInstance().addConfByKey(3, "defaultUserPricing",
				"39.95", null, "default Price in USD for a User per Year");
		
		Configurationmanagement.getInstance().addConfByKey(3, "discountNumberOfUsers1",
				"10", null, "default Pricing Barrier one");
		Configurationmanagement.getInstance().addConfByKey(3, "discountAmount1",
				"10", null, "default Discount Barrier one");
		
		Configurationmanagement.getInstance().addConfByKey(3, "discountNumberOfUsers2",
				"20", null, "default Pricing Barrier two");
		Configurationmanagement.getInstance().addConfByKey(3, "discountAmount2",
				"20", null, "default Discount Barrier two");
		
		Configurationmanagement.getInstance().addConfByKey(3, "maxNumberOfUser",
				"30", null, "default Discount Barrier three, values greater then that will receive a info message");
		
		Configurationmanagement.getInstance().addConfByKey(3, "baseUrlPaypal",
				"https://localhost:8443/paypal/", null, 
				"default Port of Paypal application");
		
		Configurationmanagement.getInstance().addConfByKey(3, "invoiceNumber",
				"0", null, "Current Invoice Number");
		
		Configurationmanagement.getInstance().addConfByKey(3, "invoiceDescriptionSingleUser",
				"i4Change License 1 Year Single User", null, "invoiceDescriptionSingleUser");
		
		Configurationmanagement.getInstance().addConfByKey(3, "invoiceDescriptionMultiUser",
				"i4Change License 1 Year Multi User", null, "invoiceDescriptionMultiUser");
		
		//86400000 == 1 Day
		
		DiagramTypeDaoImpl.getInstance().addDiagramType(new Long(593),"Process-Tree");//Process-Tree
		DiagramTypeDaoImpl.getInstance().addDiagramType(new Long(594),"Flow");//Flow
		DiagramTypeDaoImpl.getInstance().addDiagramType(new Long(595),"Organization");//Organization
		DiagramTypeDaoImpl.getInstance().addDiagramType(new Long(726),"MyRole");//MyRole
		...
		ReportTypesDaoImpl.getInstance().addReportType("overview");
	...
	public void loadInitUserAndOrganisation(String username, String userpass,
		...
		//Add user
		try {
			Long user_id = Usermanagement.getInstance().importUserInstallation(
			...
			//Add default group
			Long organisation_id = OrganisationDaoImpl.getInstance()
					.addOrganisation(defaultOrganisationName, user_id);

			//Add user to default group
			OrganisationDaoImpl.getInstance().addUserToOrganisation(
	...
	private void loadCountriesFiles(String filePath) throws Exception {
		...
		for (Iterator it = root.elementIterator("country"); it.hasNext();) {
			...
			CountryDaoImpl.getInstance().addCountry(country, paypal);
	...
	public void loadInitLanguages(String filePath) throws Exception {
		...
		for (Iterator it = roothelp.elementIterator("item"); it.hasNext();) {
			...
			HelpTopicServiceDaoImpl.getInstance().addHelpTopic(labelId, topicLabelId, helpId, helpName, isAgentHelpBool, priority);
