...
Debug.ignoringErrors = 
/* -*- file: compiler/LzRuntime.lzs#394.28 -*- */
function ignoringErrors ($1_closure, $2_context, $3_errval) {
try {
...
Instance.prototype.addProperty("$lzsc$initialize", 
/* -*- file: compiler/Class.lzs#152.52 -*- */
function $lzsc$initialize () {
...
LzMessage = Class.make("LzMessage", null, ["message", "", "length", 0, "$lzsc$initialize", 
/* -*- file: compiler/LzFormatter.lzs#24.3 -*- */
function $lzsc$initialize ($1_message) {
...
LzFormatter = Trait.make("LzFormatter", null, ["pad", 
...
function inspect ($1_obj, $2_verbose) {
...
__LzLeak = Class.make("__LzLeak", null, ["obj", void 0, "path", "", "parent", void 0, "property", "", "leaked", 0, "$lzsc$initialize", 
/* -*- file: debugger/LzMemory.lzs#494.3 -*- */
function $lzsc$initialize ($1_o) {
...
__LzLeaks = Class.make("__LzLeaks", null, ["length", 0, "sort", Array.prototype.sort, "$lzsc$initialize", 
...
LzNode = Class.make("LzNode", null, ["$lzsc$initialize", 
/* -*- file: core/LzNode.lzs#50.1 -*- */
function $lzsc$initialize ($1_parent, $2_attrs, $3_children, $4_instcall) {
...
if ($5_v != null) {
if (!this.__LZdeleted) {
var $lzsc$1101655898 = this.setters;
if ($lzsc$1101655898 && $4_k in $lzsc$1101655898) {
this[$lzsc$1101655898[$4_k]]($5_v)
} else {
if ($lzsc$1101655898 == null) {
Debug.warn("null setters on", this, $4_k, $5_v)
};
this[$4_k] = $5_v;
var $lzsc$694368423 = "on" + $4_k;
if ($lzsc$694368423 in this) {
if (this[$lzsc$694368423].ready) {
this[$lzsc$694368423].sendEvent($5_v)
}}}}}}}};
...
if (!this.__LZdeleted) {
var $lzsc$570869631 = this.setters;
if ($lzsc$570869631 && $2_k in $lzsc$570869631) {
this[$lzsc$570869631[$2_k]]($4_v)
} else {
if ($lzsc$570869631 == null) {
Debug.warn("null setters on", this, $2_k, $4_v)
};
this[$2_k] = $4_v;
var $lzsc$1713205881 = "on" + $2_k;
if ($lzsc$1713205881 in this) {
if (this[$lzsc$1713205881].ready) {
this[$lzsc$1713205881].sendEvent($4_v)
}}}}}}, "construct", 
...
}}}, "ignoreplacement", null, "__LZapplyArgs", 
...
if (!this.__LZdeleted) {
var $lzsc$1629637926 = this.setters;
if ($lzsc$1629637926 && $1_prop in $lzsc$1629637926) {
this[$lzsc$1629637926[$1_prop]]($2_val)
} else {
if ($lzsc$1629637926 == null) {
Debug.warn("null setters on", this, $1_prop, $2_val)
};
this[$1_prop] = $2_val;
var $lzsc$278136646 = "on" + $1_prop;
if ($lzsc$278136646 in this) {
if (this[$lzsc$278136646].ready) {
this[$lzsc$278136646].sendEvent($2_val)
...
return $5_w
}, "setters", {name: "setName", id: "setID", $refs: "__LZstoreRefs", $delegates: "__LZstoreDelegates", options: "__LZsetOptions", placement: -1, datapath: "setDatapath", $setters: -1, $classrootdepth: "__LZsetClassRoot", $datapath: "__LZmakeDatapath"}, "__LZsetClassRoot", 
...
if (!this.__LZdeleted) {
var $lzsc$383413943 = this.setters;
if ($lzsc$383413943 && $1_prop in $lzsc$383413943) {
this[$lzsc$383413943[$1_prop]]($6_val)
} else {
if ($lzsc$383413943 == null) {
Debug.warn("null setters on", this, $1_prop, $6_val)
};
this[$1_prop] = $6_val;
var $lzsc$278382468 = "on" + $1_prop;
if ($lzsc$278382468 in this) {
if (this[$lzsc$278382468].ready) {
this[$lzsc$278382468].sendEvent($6_val)
}}}};
return null
};
...
LzUserClass = Class.make("LzUserClass", LzNode, ["$lzsc$initialize", 
/* -*- file: core/UserClass.lzs#77.1 -*- */
function $lzsc$initialize ($1_parent, $2_args) {
...
LzDelegate = Class.make("LzDelegate", null, ["$lzsc$initialize", 
/* -*- file: events/LaszloEvents.lzs#35.1 -*- */
function $lzsc$initialize ($1_context, $2_functionName, $3_eventSender, $4_eventName) {
(arguments.callee.superclass ? arguments.callee.superclass.prototype["$lzsc$initialize"] : this.nextMethod(arguments.callee, "$lzsc$initialize")).call(this);
this.c = $1_context;
...
LzEvent = Class.make("LzEvent", null, ["$lzsc$initialize", 
/* -*- file: events/LaszloEvents.lzs#358.1 -*- */
function $lzsc$initialize ($1_eventSender, $2_eventName, $3_d) {
(arguments.callee.superclass ? arguments.callee.superclass.prototype["$lzsc$initialize"] : this.nextMethod(arguments.callee, "$lzsc$initialize")).call(this);
...
LzLibraryCleanup = Class.make("LzLibraryCleanup", LzNode, ["$lzsc$initialize", 
/* -*- file: kernel/LzLibraryCleanup.lzs#22.5 -*- */
function $lzsc$initialize ($1_owner, $2_args) {
...
LzLibrary = Class.make("LzLibrary", LzNode, ["construct", 
...
if (this.stage == "late") {
this.load()
}}, "loaded", false, "loading", false, "onload", LzDeclaredEvent, "toString", 
...
LzContextMenu = Class.make("LzContextMenu", LzNode, ["$lzsc$initialize", 
/* -*- file: kernel/dhtml/LzContextMenu.js#22.1 -*- */
function $lzsc$initialize ($1_del) {
this.__LZmousedowndel = new LzDelegate(this, "__hide");
this.items = [];
this.setDelegate($1_del)
}, "onmenuopen", LzDeclaredEvent, "showbuiltins", false, "setDelegate", 
...
LzContextMenuItem = Class.make("LzContextMenuItem", LzNode, ["$lzsc$initialize", 
/* -*- file: kernel/dhtml/LzContextMenu.js#168.1 -*- */
function $lzsc$initialize ($1_title, $2_del) {
this.cmenuitem = {visible: true, enabled: true, separatorBefore: false, caption: $1_title};
this.setDelegate($2_del)
}, "onselect", LzDeclaredEvent, "setDelegate", 
...
if (this[$1_attrname]) {
if (!this.__LZdeleted) {
var $lzsc$195188125 = this.setters;
if ($lzsc$195188125 && $1_attrname in $lzsc$195188125) {
this[$lzsc$195188125[$1_attrname]]($2_value)
} else {
if ($lzsc$195188125 == null) {
Debug.warn("null setters on", this, $1_attrname, $2_value)
};
this[$1_attrname] = $2_value;
var $lzsc$470554554 = "on" + $1_attrname;
if ($lzsc$470554554 in this) {
if (this[$lzsc$470554554].ready) {
this[$lzsc$470554554].sendEvent($2_value)
}}}}}}, "__makeSprite", 
...
function resourceevent ($1_name, $2_value, $3_eventonly) {
if ($3_eventonly != true) {
this[$1_name] = $2_value
};
...
if (!this.__LZdeleted) {
var $lzsc$101125124 = this.setters;
if ($lzsc$101125124 && "x" in $lzsc$101125124) {
this[$lzsc$101125124["x"]](0)
} else {
if ($lzsc$101125124 == null) {
...
function  () {
var $lzsc$1128549631 = this.immediateparent.width / 2 - this.width / 2;
if (!this.__LZdeleted) {
var $lzsc$763069982 = this.setters;
if ($lzsc$763069982 && "x" in $lzsc$763069982) {
this[$lzsc$763069982["x"]]($lzsc$1128549631)
} else {
if ($lzsc$763069982 == null) {
Debug.warn("null setters on", this, "x", $lzsc$1128549631)
};
this["x"] = $lzsc$1128549631;
if ("onx" in this) {
if (this["onx"].ready) {
this["onx"].sendEvent($lzsc$1128549631)
}}}}};
...
function  () {
var $lzsc$1055756328 = this.immediateparent.width - this.width;
if (!this.__LZdeleted) {
var $lzsc$164128453 = this.setters;
if ($lzsc$164128453 && "x" in $lzsc$164128453) {
this[$lzsc$164128453["x"]]($lzsc$1055756328)
} else {
if ($lzsc$164128453 == null) {
Debug.warn("null setters on", this, "x", $lzsc$1055756328)
};
this["x"] = $lzsc$1055756328;
if ("onx" in this) {
if (this["onx"].ready) {
this["onx"].sendEvent($lzsc$1055756328)
}}}}};
...
if (!this.__LZdeleted) {
var $lzsc$1459452032 = this.setters;
if ($lzsc$1459452032 && "y" in $lzsc$1459452032) {
this[$lzsc$1459452032["y"]](0)
} else {
if ($lzsc$1459452032 == null) {
...
function  () {
var $lzsc$66633470 = this.immediateparent.height / 2 - this.height / 2;
if (!this.__LZdeleted) {
var $lzsc$1981049850 = this.setters;
if ($lzsc$1981049850 && "y" in $lzsc$1981049850) {
this[$lzsc$1981049850["y"]]($lzsc$66633470)
} else {
if ($lzsc$1981049850 == null) {
Debug.warn("null setters on", this, "y", $lzsc$66633470)
};
this["y"] = $lzsc$66633470;
if ("ony" in this) {
if (this["ony"].ready) {
this["ony"].sendEvent($lzsc$66633470)
}}}}};
...
function  () {
var $lzsc$2132012229 = this.immediateparent.height - this.height;
if (!this.__LZdeleted) {
var $lzsc$489571375 = this.setters;
if ($lzsc$489571375 && "y" in $lzsc$489571375) {
this[$lzsc$489571375["y"]]($lzsc$2132012229)
} else {
if ($lzsc$489571375 == null) {
Debug.warn("null setters on", this, "y", $lzsc$2132012229)
};
this["y"] = $lzsc$2132012229;
if ("ony" in this) {
if (this["ony"].ready) {
this["ony"].sendEvent($lzsc$2132012229)
}}}}};
...
var $lzsc$729543544 = ($4_val - $3_tLink.offset[$1_prop]) / $3_tLink.scale[$1_prop];
if (!this.__LZdeleted) {
var $lzsc$479000237 = this.setters;
if ($lzsc$479000237 && $1_prop in $lzsc$479000237) {
this[$lzsc$479000237[$1_prop]]($lzsc$729543544)
} else {
if ($lzsc$479000237 == null) {
Debug.warn("null setters on", this, $1_prop, $lzsc$729543544)
};
this[$1_prop] = $lzsc$729543544;
var $lzsc$1952254974 = "on" + $1_prop;
if ($lzsc$1952254974 in this) {
if (this[$lzsc$1952254974].ready) {
this[$lzsc$1952254974].sendEvent($lzsc$729543544)
}}}}} else {
if ($1_prop == "width" || $1_prop == "height") {
var $5_axis = $1_prop == "width" ? "x" : "y";
$3_tLink.update($5_axis);
var $lzsc$442445395 = $4_val / $3_tLink.scale[$5_axis];
if (!this.__LZdeleted) {
var $lzsc$2026304348 = this.setters;
if ($lzsc$2026304348 && $1_prop in $lzsc$2026304348) {
this[$lzsc$2026304348[$1_prop]]($lzsc$442445395)
} else {
if ($lzsc$2026304348 == null) {
Debug.warn("null setters on", this, $1_prop, $lzsc$442445395)
};
this[$1_prop] = $lzsc$442445395;
var $lzsc$652345271 = "on" + $1_prop;
if ($lzsc$652345271 in this) {
if (this[$lzsc$652345271].ready) {
this[$lzsc$652345271].sendEvent($lzsc$442445395)
}}}}} else {
...
LzViewLinkage = Class.make("LzViewLinkage", null, ["$lzsc$initialize", 
/* -*- file: views/LzViewLinkage.lzs#41.3 -*- */
function $lzsc$initialize ($1_fromView, $2_toView) {
...
LzText = Class.make("LzText", [LzFormatter, LzView], ["ontext", LzDeclaredEvent, "onmaxlength", LzDeclaredEvent, "onpattern", LzDeclaredEvent, "onscroll", LzDeclaredEvent, "onmaxscroll", LzDeclaredEvent, "onhscroll", LzDeclaredEvent, "onmaxhscroll", LzDeclaredEvent, "scroll", 0, "maxscroll", 0, "hscroll", 0, "maxhscroll", 0, "getDefaultWidth", 
...
function getMCRef () {
return this.sprite.getMCRef()
}, "maxlength", null, "pattern", null, "setResize", 
...
LzInputText = Class.make("LzInputText", LzText, ["onenabled", LzDeclaredEvent, "onselect", LzDeclaredEvent, "ontext", LzDeclaredEvent, "getDefaultWidth", 
...
if (!this.__LZdeleted) {
var $lzsc$1726614389 = this.setters;
if ($lzsc$1726614389 && "focusable" in $lzsc$1726614389) {
this[$lzsc$1726614389["focusable"]]($1_enabled)
} else {
if ($lzsc$1726614389 == null) {
...
function getText () {
return this.sprite.getText()
}, "_allowselectable", true, "_selectable", null, "_modechanged", 
/* -*- file: views/LzInputText.lzs#255.1 -*- */
function _modechanged ($1_modalview) {
if (!$1_modalview) {
...
LzCanvas = Class.make("LzCanvas", LzView, ["$lzsc$initialize", 
/* -*- file: views/LaszloCanvas.lzs#64.1 -*- */
function $lzsc$initialize ($1_args) {
...
LzScript = Class.make("LzScript", LzNode, ["$lzsc$initialize", 
...
function setDuration ($1_duration) {
...
LzAnimator = Class.make("LzAnimator", LzAnimatorGroup, ["beginPoleDelta", 0.25, "endPoleDelta", 0.25, "attribute", null, "construct", 
...
this.crepeat = this.repeat;
if (this.from != null) {
var $lzsc$607279633 = this.target;
var $lzsc$1927202108 = this.attribute;
var $lzsc$425302652 = Number(this.from);
if (!$lzsc$607279633.__LZdeleted) {
var $lzsc$126997078 = $lzsc$607279633.setters;
if ($lzsc$126997078 && $lzsc$1927202108 in $lzsc$126997078) {
$lzsc$607279633[$lzsc$126997078[$lzsc$1927202108]]($lzsc$425302652)
} else {
if ($lzsc$126997078 == null) {
Debug.warn("null setters on", $lzsc$607279633, $lzsc$1927202108, $lzsc$425302652)
};
$lzsc$607279633[$lzsc$1927202108] = $lzsc$425302652;
var $lzsc$1759944803 = "on" + $lzsc$1927202108;
if ($lzsc$1759944803 in $lzsc$607279633) {
if ($lzsc$607279633[$lzsc$1759944803].ready) {
$lzsc$607279633[$lzsc$1759944803].sendEvent($lzsc$425302652)
}}}}};
if (this.relative) {
this.to = this.origto
} else {
...
function resetAnimator () {
if (this.from != null) {
var $lzsc$1148815892 = this.target;
var $lzsc$1462206683 = this.attribute;
var $lzsc$717887782 = this.from;
if (!$lzsc$1148815892.__LZdeleted) {
var $lzsc$184294684 = $lzsc$1148815892.setters;
if ($lzsc$184294684 && $lzsc$1462206683 in $lzsc$184294684) {
$lzsc$1148815892[$lzsc$184294684[$lzsc$1462206683]]($lzsc$717887782)
} else {
if ($lzsc$184294684 == null) {
Debug.warn("null setters on", $lzsc$1148815892, $lzsc$1462206683, $lzsc$717887782)
};
$lzsc$1148815892[$lzsc$1462206683] = $lzsc$717887782;
var $lzsc$702145307 = "on" + $lzsc$1462206683;
if ($lzsc$702145307 in $lzsc$1148815892) {
if ($lzsc$1148815892[$lzsc$702145307].ready) {
$lzsc$1148815892[$lzsc$702145307].sendEvent($lzsc$717887782)
}}}};
...
if (this.target[$1_e_prop].c <= 0) {
this.target[$1_e_prop].c = 0;
var $lzsc$608310775 = this.target;
var $lzsc$2113152840 = this.attribute;
var $lzsc$746291656 = this.target[$1_e_prop].v;
if (!$lzsc$608310775.__LZdeleted) {
var $lzsc$1522368252 = $lzsc$608310775.setters;
if ($lzsc$1522368252 && $lzsc$2113152840 in $lzsc$1522368252) {
$lzsc$608310775[$lzsc$1522368252[$lzsc$2113152840]]($lzsc$746291656)
} else {
if ($lzsc$1522368252 == null) {
Debug.warn("null setters on", $lzsc$608310775, $lzsc$2113152840, $lzsc$746291656)
};
$lzsc$608310775[$lzsc$2113152840] = $lzsc$746291656;
var $lzsc$1621697522 = "on" + $lzsc$2113152840;
if ($lzsc$1621697522 in $lzsc$608310775) {
if ($lzsc$608310775[$lzsc$1621697522].ready) {
$lzsc$608310775[$lzsc$1621697522].sendEvent($lzsc$746291656)
}}}};
this.target[$1_e_prop].v = null
};
this.__LZhalt()
...
var $lzsc$1369587393 = this.target;
var $lzsc$1983219883 = this.attribute;
var $lzsc$720695325 = this.target[this.attribute] + $2_aDiff;
if (!$lzsc$1369587393.__LZdeleted) {
var $lzsc$724701289 = $lzsc$1369587393.setters;
if ($lzsc$724701289 && $lzsc$1983219883 in $lzsc$724701289) {
$lzsc$1369587393[$lzsc$724701289[$lzsc$1983219883]]($lzsc$720695325)
} else {
if ($lzsc$724701289 == null) {
Debug.warn("null setters on", $lzsc$1369587393, $lzsc$1983219883, $lzsc$720695325)
};
$lzsc$1369587393[$lzsc$1983219883] = $lzsc$720695325;
var $lzsc$1785447341 = "on" + $lzsc$1983219883;
if ($lzsc$1785447341 in $lzsc$1369587393) {
if ($lzsc$1369587393[$lzsc$1785447341].ready) {
$lzsc$1369587393[$lzsc$1785447341].sendEvent($lzsc$720695325)
}}}}};
...
LzLayout = Class.make("LzLayout", LzNode, ["locked", 2, "subviews", null, "updateDelegate", null, "delegates", null, "construct", 
...
LzFont = Class.make("LzFont", null, ["style", null, "name", null, "height", null, "ascent", null, "descent", null, "advancetable", null, "lsbtable", null, "rsbtable", null, "$lzsc$initialize", 
/* -*- file: helpers/LzFont.lzs#365.1 -*- */
function $lzsc$initialize ($1_fontobject, $2_attrs, $3_style) {
(arguments.callee.superclass ? arguments.callee.superclass.prototype["$lzsc$initialize"] : this.nextMethod(arguments.callee, "$lzsc$initialize")).call(this);
...
LzSelectionManager = Class.make("LzSelectionManager", LzNode, ["sel", null, "selectedHash", null, "selected", null, "construct", 
...
LzDataSelectionManager = Class.make("LzDataSelectionManager", LzSelectionManager, ["makeSelected", 
...
LzCommand = Class.make("LzCommand", LzNode, ["active", true, "keys", null, "onselect", LzDeclaredEvent, "setKeys", 
...
LzState = Class.make("LzState", LzNode, ["onapply", LzDeclaredEvent, "onremove", LzDeclaredEvent, "asyncnew", false, "subh", null, "pooling", false, "construct", 
...
LzDataNode = Trait.make("LzDataNode", null, ["onownerDocument", LzDeclaredEvent, "nodeType", null, "getPreviousSibling", 
...
LzDataElementTrait = Trait.make("LzDataElementTrait", null, ["onDocumentChange", LzDeclaredEvent, "onparentNode", LzDeclaredEvent, "onchildNode", LzDeclaredEvent, "onchildNodes", LzDeclaredEvent, "onattributes", LzDeclaredEvent, "onnodeName", LzDeclaredEvent, "__LZo", -1, "__LZcoDirty", true, "__LZchangeQ", null, "__LZlocker", null, "insertBefore", 
...
LzMiniNode = Class.make("LzMiniNode", null, ["setAttribute", LzNode.prototype.setAttribute], ["initialize", 
/* -*- file: data/LzDataElement.lzs#621.10 -*- */
function initialize ($1_prototype) {
if (!$1_prototype.hasOwnProperty("setters")) {
$1_prototype["setters"] = new LzInheritedHash($1_prototype["setters"])
}}]);
LzDataElement = Class.make("LzDataElement", [LzDataElementTrait, LzDataNode, LzMiniNode], ["$lzsc$initialize", 
/* -*- file: data/LzDataElement.lzs#642.5 -*- */
function $lzsc$initialize ($1_name, $2_attributes, $3_children) {
...
LzDataText = Class.make("LzDataText", [LzDataNode, LzMiniNode], ["$lzsc$initialize", 
/* -*- file: data/LzDataText.lzs#24.1 -*- */
function $lzsc$initialize ($1_text) {
(arguments.callee.superclass ? arguments.callee.superclass.prototype["$lzsc$initialize"] : this.nextMethod(arguments.callee, "$lzsc$initialize")).call(this);
this.data = $1_text
}, "onDocumentChange", LzDeclaredEvent, "onparentNode", LzDeclaredEvent, "onchildNode", LzDeclaredEvent, "onchildNodes", LzDeclaredEvent, "onattributes", LzDeclaredEvent, "onnodeName", LzDeclaredEvent, "nodeType", LzDataNode.TEXT_NODE, "nodeName", "#text", "data", "", "length", 0, "setData", 
...
LzDataRequest = Class.make("LzDataRequest", LzNode, ["requestor", null, "src", null, "timeout", Infinity, "status", null, "rawdata", null, "error", null, "onstatus", LzDeclaredEvent, "$lzsc$initialize", 
/* -*- file: data/LzDataRequest.lzs#49.5 -*- */
function $lzsc$initialize ($1_requestor) {
...
LzDataProvider = Class.make("LzDataProvider", LzNode, ["doRequest", 
/* -*- file: data/LzDataProvider.lzs#32.5 -*- */
function doRequest ($1_dreq) {

}], ["tagname", "dataprovider"]);
LzHTTPDataProvider = Class.make("LzHTTPDataProvider", LzDataProvider, ["makeLoader", 
...
}], ["tagname", "dataset", "queryStringToTable", 
/* -*- file: data/LzDataset.lzs#598.8 -*- */
function queryStringToTable ($1_query) {
var $2_queries = {};
...
__LzHttpDatasetPoolClass = Class.make("__LzHttpDatasetPoolClass", null, ["_uid", 0, "_p", [], "get", 
...
LzDatapointer = Class.make("LzDatapointer", LzNode, ["p", null, "data", null, "context", null, "__LZtracking", null, "__LZtrackDel", null, "rerunxpath", false, "onp", LzDeclaredEvent, "onDocumentChange", LzDeclaredEvent, "ondata", LzDeclaredEvent, "onerror", LzDeclaredEvent, "ontimeout", LzDeclaredEvent, "onrerunxpath", LzDeclaredEvent, "childOrSelf", 
...
LzParam = Class.make("LzParam", LzNode, ["construct", 
/* -*- file: data/LzParam.lzs#53.1 -*- */
function construct ($1_parent, $2_args) {
(arguments.callee.superclass ? arguments.callee.superclass.prototype["construct"] : this.nextMethod(arguments.callee, "construct")).apply(this, arguments);
this.d = {}}, "createChildren", 
...
return $5_o
}], ["tagname", "params", "parseQueryString", 
...
LzParsedPath = Class.make("LzParsedPath", null, ["path", null, "selectors", null, "context", null, "dsetname", null, "dsrcname", null, "islocaldata", null, "operator", null, "aggOperator", null, "operatorArgs", null, "hasAttrOper", false, "hasOpSelector", false, "hasDotDot", false, "getContext", 
/* -*- file: data/LzParsedPath.lzs#111.3 -*- */
function getContext ($1_dp) {
...
function $lzsc$initialize ($1_pa, $2_node) {
this.path = $1_pa;
this.selectors = [];
var $3_nowarn = false;
...
AnonDatasetGenerator = Class.make("AnonDatasetGenerator", null, ["$lzsc$initialize", 
/* -*- file: data/LzParsedPath.lzs#378.1 -*- */
function $lzsc$initialize ($1_pp) {
this.pp = $1_pp
}, "getContext", 
...
LzDatapath = Class.make("LzDatapath", LzDatapointer, ["datacontrolsvisibility", true, "__LZtakeDPSlot", true, "pooling", null, "replication", null, "sortpath", null, "sortorder", null, "construct", 
...
LzReplicationManager = Class.make("LzReplicationManager", LzDatapath, ["pooling", false, "asyncnew", true, "datacontrolsvisibility", false, "__LZtakeDPSlot", false, "visible", true, "__LZpreventXPathUpdate", false, "nodes", null, "clones", null, "__LZdataoffset", 0, "onnodes", LzDeclaredEvent, "onclones", LzDeclaredEvent, "onvisible", LzDeclaredEvent, "construct", 
...
LzDatasource = Class.make("LzDatasource", LzNode, ["proxied", null, "loadSuccess", 
...
LzHTTPDatasource = Class.make("LzHTTPDatasource", LzDatasource, ["reqtype", "GET", "isProxied", 
...
LzDataAttrBind = Class.make("LzDataAttrBind", LzDatapointer, ["$lzsc$initialize", 
/* -*- file: data/LzDataAttrBind.lzs#14.1 -*- */
function $lzsc$initialize ($1_ndpath, $2_attr, $3_path) {
this.setAttr = $2_attr;
...
var $lzsc$1187725062 = this.node;
var $lzsc$1623169332 = this.setAttr;
var $lzsc$1122768956 = this.data == null ? null : this.data;
if (!$lzsc$1187725062.__LZdeleted) {
var $lzsc$589479515 = $lzsc$1187725062.setters;
if ($lzsc$589479515 && $lzsc$1623169332 in $lzsc$589479515) {
$lzsc$1187725062[$lzsc$589479515[$lzsc$1623169332]]($lzsc$1122768956)
} else {
if ($lzsc$589479515 == null) {
Debug.warn("null setters on", $lzsc$1187725062, $lzsc$1623169332, $lzsc$1122768956)
};
$lzsc$1187725062[$lzsc$1623169332] = $lzsc$1122768956;
var $lzsc$994183441 = "on" + $lzsc$1623169332;
if ($lzsc$994183441 in $lzsc$1187725062) {
if ($lzsc$1187725062[$lzsc$994183441].ready) {
$lzsc$1187725062[$lzsc$994183441].sendEvent($lzsc$1122768956)
}}}}}}, "unregisterAll", 
...
LzLazyReplicationManager = Class.make("LzLazyReplicationManager", LzReplicationManager, ["axis", "y", "spacing", 0, "mask", null, "construct", 
...
var $lzsc$1335645815 = this.axis;
var $lzsc$831406601 = this.totalsize;
if (!$7_v.__LZdeleted) {
var $lzsc$379639041 = $7_v.setters;
if ($lzsc$379639041 && $lzsc$1335645815 in $lzsc$379639041) {
$7_v[$lzsc$379639041[$lzsc$1335645815]]($lzsc$831406601)
} else {
if ($lzsc$379639041 == null) {
Debug.warn("null setters on", $7_v, $lzsc$1335645815, $lzsc$831406601)
};
$7_v[$lzsc$1335645815] = $lzsc$831406601;
var $lzsc$1980415868 = "on" + $lzsc$1335645815;
if ($lzsc$1980415868 in $7_v) {
if ($7_v[$lzsc$1980415868].ready) {
$7_v[$lzsc$1980415868].sendEvent($lzsc$831406601)
}}}};
...
var $lzsc$1103419064 = this.cloneimmediateparent;
var $lzsc$555603285 = this.sizeAxis;
var $lzsc$1313339039 = this.nodes.length * this.totalsize - this.spacing;
if (!$lzsc$1103419064.__LZdeleted) {
var $lzsc$1895579229 = $lzsc$1103419064.setters;
if ($lzsc$1895579229 && $lzsc$555603285 in $lzsc$1895579229) {
$lzsc$1103419064[$lzsc$1895579229[$lzsc$555603285]]($lzsc$1313339039)
} else {
if ($lzsc$1895579229 == null) {
Debug.warn("null setters on", $lzsc$1103419064, $lzsc$555603285, $lzsc$1313339039)
};
$lzsc$1103419064[$lzsc$555603285] = $lzsc$1313339039;
var $lzsc$86240303 = "on" + $lzsc$555603285;
if ($lzsc$86240303 in $lzsc$1103419064) {
if ($lzsc$1103419064[$lzsc$86240303].ready) {
$lzsc$1103419064[$lzsc$86240303].sendEvent($lzsc$1313339039)
}}}};
...
var $lzsc$1011849248 = this.axis;
var $lzsc$1782408171 = ($11_i + $4_newstart) * this.totalsize;
if (!$12_cl.__LZdeleted) {
var $lzsc$2030209391 = $12_cl.setters;
if ($lzsc$2030209391 && $lzsc$1011849248 in $lzsc$2030209391) {
$12_cl[$lzsc$2030209391[$lzsc$1011849248]]($lzsc$1782408171)
} else {
if ($lzsc$2030209391 == null) {
Debug.warn("null setters on", $12_cl, $lzsc$1011849248, $lzsc$1782408171)
};
$12_cl[$lzsc$1011849248] = $lzsc$1782408171;
var $lzsc$1160349883 = "on" + $lzsc$1011849248;
if ($lzsc$1160349883 in $12_cl) {
if ($12_cl[$lzsc$1160349883].ready) {
$12_cl[$lzsc$1160349883].sendEvent($lzsc$1782408171)
}}}};
...
LzResizeReplicationManager = Class.make("LzResizeReplicationManager", LzLazyReplicationManager, ["construct", 
...
this.clones[$14_i] = $23_cl;
var $lzsc$2017144629 = this.axis;
if (!$23_cl.__LZdeleted) {
var $lzsc$1665567135 = $23_cl.setters;
if ($lzsc$1665567135 && $lzsc$2017144629 in $lzsc$1665567135) {
$23_cl[$lzsc$1665567135[$lzsc$2017144629]]($24_clpos)
} else {
if ($lzsc$1665567135 == null) {
Debug.warn("null setters on", $23_cl, $lzsc$2017144629, $24_clpos)
};
$23_cl[$lzsc$2017144629] = $24_clpos;
var $lzsc$1750113686 = "on" + $lzsc$2017144629;
if ($lzsc$1750113686 in $23_cl) {
if ($23_cl[$lzsc$1750113686].ready) {
$23_cl[$lzsc$1750113686].sendEvent($24_clpos)
}}}};
var $16_ds = $15_cnode[this.datasizevar];
var $17_csiz = $16_ds == null ? this.viewsize : $16_ds;
if ($23_cl[this.sizeAxis] != $17_csiz) {
var $lzsc$1224864790 = this.sizeAxis;
if (!($23_cl.__LZdeleted || true && $23_cl[$lzsc$1224864790] == $17_csiz)) {
var $lzsc$153174090 = $23_cl.setters;
if ($lzsc$153174090 && $lzsc$1224864790 in $lzsc$153174090) {
$23_cl[$lzsc$153174090[$lzsc$1224864790]]($17_csiz)
} else {
if ($lzsc$153174090 == null) {
Debug.warn("null setters on", $23_cl, $lzsc$1224864790, $17_csiz)
};
$23_cl[$lzsc$1224864790] = $17_csiz;
var $lzsc$2050367366 = "on" + $lzsc$1224864790;
if ($lzsc$2050367366 in $23_cl) {
if ($23_cl[$lzsc$2050367366].ready) {
$23_cl[$lzsc$2050367366].sendEvent($17_csiz)
}}}}};
$24_clpos += $17_csiz + this.spacing
};
this.__LZdataoffset = $6_newoffset;
var $lzsc$780783791 = this.cloneimmediateparent;
var $lzsc$612241263 = this.sizeAxis;
if (!$lzsc$780783791.__LZdeleted) {
var $lzsc$1443599212 = $lzsc$780783791.setters;
if ($lzsc$1443599212 && $lzsc$612241263 in $lzsc$1443599212) {
$lzsc$780783791[$lzsc$1443599212[$lzsc$612241263]]($10_cpos)
} else {
if ($lzsc$1443599212 == null) {
Debug.warn("null setters on", $lzsc$780783791, $lzsc$612241263, $10_cpos)
};
$lzsc$780783791[$lzsc$612241263] = $10_cpos;
var $lzsc$487243056 = "on" + $lzsc$612241263;
if ($lzsc$487243056 in $lzsc$780783791) {
if ($lzsc$780783791[$lzsc$487243056].ready) {
$lzsc$780783791[$lzsc$487243056].sendEvent($10_cpos)
}}}};
...
LzInstantiatorClass = Class.make("LzInstantiatorClass", null, ["checkQDel", null, "$lzsc$initialize", 
...
for (var $3_u in $2_h) {
var $1_o = $2_h[$3_u];
var $lzsc$2070247457 = global[$1_o.c];
var $lzsc$1582519880 = $1_o.n;
var $lzsc$1151392618 = $1_o.v;
if (!$lzsc$2070247457.__LZdeleted) {
var $lzsc$465935486 = $lzsc$2070247457.setters;
if ($lzsc$465935486 && $lzsc$1582519880 in $lzsc$465935486) {
$lzsc$2070247457[$lzsc$465935486[$lzsc$1582519880]]($lzsc$1151392618)
} else {
if ($lzsc$465935486 == null) {
Debug.warn("null setters on", $lzsc$2070247457, $lzsc$1582519880, $lzsc$1151392618)
};
$lzsc$2070247457[$lzsc$1582519880] = $lzsc$1151392618;
var $lzsc$371277746 = "on" + $lzsc$1582519880;
if ($lzsc$371277746 in $lzsc$2070247457) {
if ($lzsc$2070247457[$lzsc$371277746].ready) {
$lzsc$2070247457[$lzsc$371277746].sendEvent($lzsc$1151392618)
}}}}}};
...
LzTrackClass = Class.make("LzTrackClass", null, ["__LZreg", new Object(), "__LZactivegroups", null, "__LZtrackDel", null, "__LZoptimizeforaxis", "x", "__LZmouseupDel", null, "$lzsc$initialize", 
...
LzIdleClass = Class.make("LzIdleClass", null, ["coi", null, "removeCOI", null, "$lzsc$initialize", 
...
this.regNext = false
}}, "onidle", LzDeclaredEvent, "toString", 
...
LzTimerClass = Class.make("LzTimerClass", null, ["timerList", new Object(), "addTimer", 
