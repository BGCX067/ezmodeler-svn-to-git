...
public class EmailDaoImpl {
	...
	private static EmailDaoImpl instance = null;
	...
	public String sendMail(String Username, String Userpass, String EMail, String activation_key) {
		...
		if (sendEmailAtRegister==1){
			...
			String template = RegisterUserTemplate.getInstance().getRegisterUserTemplate(Username,Userpass,EMail,default_lang_id, activation_key);
			Fieldlanguagesvalues label = Fieldmanagment.getInstance().getFieldByLabelNumberAndLanguage(new Long(512), default_lang_id);
			
			succ = MailHandler.sendMail(EMail, label.getValue(), template, null);
			return succ;
		} else {
			return "success";
		}
