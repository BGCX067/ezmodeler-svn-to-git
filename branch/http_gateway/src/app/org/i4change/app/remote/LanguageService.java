...
public class LanguageService {
	 ...
	 * @return
	 */
	public SearchResult getFieldsByLanguage(String SID, int start, int max, String orderby, boolean asc, Long language_id, String search){
        Long users_id = Sessionmanagement.getInstance().checkSession(SID);
        Long user_level = UserDaoImpl.getInstance().getUserLevelByID(users_id);
        if (AuthLevelmanagement.getInstance().checkAdminLevel(user_level)){
        	if (search.length() == 0) {
        		...
        		return Fieldmanagment.getInstance().getFieldsByLanguageBySearch(start, max, orderby, asc, language_id, search);
        	}
        }
		return null;
	}
